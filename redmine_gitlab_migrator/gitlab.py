...
class GitlabProject(Project):
    ...
    def create_issue(self, data, meta, auth_header):
        ...
        if 'human_time_estimate' in meta and meta['human_time_estimate'] is not None and meta['human_time_estimate'] != 0.0:
            time_estimate_url = '{}/time_estimate?duration={}h'.format(issue_url, meta['human_time_estimate'])
            self.api.post(time_estimate_url)
        if 'human_total_time_spent' in meta and meta['human_total_time_spent'] is not None and meta['human_total_time_spent'] != 0.0:
